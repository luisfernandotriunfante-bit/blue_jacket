...
export interface PurchaseHistory310 {
  ...
  /** Código histórico do sistema anterior. Os parsers canônicos novos sempre preenchem; opcional apenas para ler snapshots/workbooks legados durante a migração. */
  legacyProductCode?:string;
  /** @deprecated Alias de compatibilidade; nunca significa Código Winthor atual. */
  winthorCode:string;
...
export interface CustomerIntelligenceSupport { schemaVersion:1; updatedAt:string; sources:Array<{kind:string;fileName:string;loadedAt:string;note:string}>; assortmentCompetences:AssortmentCompetence[]; lineage:SkuLineageRecord[]; customers:CustomerCommercialProfile[]; purchases:PurchaseHistory310[]; promotions:PromotionRule[]; pricingRules:PricingRule[]; warnings:string[]; }
...
export interface CustomerIntelligenceResult {
  ...
  /** @deprecated aliases temporários. */
