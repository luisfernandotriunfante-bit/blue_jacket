...
export interface AssortmentCompetence {
  key: string;
  label: string;
  validFrom: string;
  validTo: string;
  sourceSheet: string;
  products: OfficialAssortmentSku[];
  expectedTotalsByChannel: Record<string, { total: number; mandatory: number; important: number }>;
}

export interface SkuLineageRecord {
  oldSku: string;
  oldEan: string;
  newSku: string;
  newEan: string;
...
export interface PricingRule {
  id: string;
  scope: 'CNPJ' | 'REDE' | 'GERAL';
  scopeKey: string;
  kind: 'ACRESCIMO' | 'RAPPEL' | 'OUTRO';
  value: number;
  validFrom: string;
  validTo: string;
  source: string;
...
export interface CustomerIntelligenceSupport {
  schemaVersion: 1;
  updatedAt: string;
  sources: Array<{ kind: string; fileName: string; loadedAt: string; note: string }>;
  assortmentCompetences: AssortmentCompetence[];
  lineage: SkuLineageRecord[];
...
export interface CustomerSourceTrace {
  field: string;
  chosen: string;
  precedence: string;
...
export interface CustomerResolvedProfile extends CustomerCommercialProfile {
  traces: CustomerSourceTrace[];
}
...
export interface ProductCommercialView {
  ...
  subcategory: string;
  brand: string;
  assortmentValue: number | null;
  classification: AssortmentClassification;
  isRecommended: boolean;
  isLaunch: boolean;
  launchLabel: string;
  lineageStatus: LineageStatus | '';
  predecessorEan: string;
  successorEan: string;
  ...
  priceStatus: 'BASE_DISPONIVEL' | 'COMPOSICAO_FINAL_PENDENTE' | 'SEM_PRECO';
...
export interface LaunchAdoptionSummary {
  totalRecommended: number;
  adopted: number;
  missing: number;
  availableNow: number;
  portfolioOnly: number;
  withoutWinthor: number;
  withoutStockAndPortfolio: number;
}

export interface CustomerIntelligenceAuditCheck {
  id: string;
  label: string;
