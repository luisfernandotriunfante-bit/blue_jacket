...
const customerKey=(tx:SalesTransaction)=>tx.customerKey || (validCnpj(tx.cnpj)?tx.cnpj:'') || (tx.clientCode?`CLIENTE:${tx.clientCode}`:`NOME:${normalizeText(tx.clientName)}`);
...
export function buildDaily(transactions: SalesTransaction[], periodStart = '', periodEnd = ''): CanonicalDailyMovement[] {
  ...
  transactions.forEach(tx => {
    ...
    if (tx.status === 'FATURADO') d.invoiced += tx.value; else d.toInvoice += tx.value;
    if (validCnpj(tx.cnpj)) {
      if (tx.status === 'FATURADO') d.invClients.add(tx.cnpj);
      d.allClients.add(tx.cnpj);
    }
  });
  if (periodStart && periodEnd) {
    const cursor = new Date(`${periodStart}T12:00:00Z`); const limit = new Date(`${periodEnd}T12:00:00Z`);
    while (cursor <= limit) { const date = cursor.toISOString().slice(0,10); if (!byDate.has(date)) byDate.set(date,{invoiced:0,toInvoice:0,invClients:new Set(),allClients:new Set()}); cursor.setUTCDate(cursor.getUTCDate()+1); }
  }
...
export function buildClients(transactions: SalesTransaction[], premisesByCnpj: Map<string, PremiseClient>, routeStores:RouteStore[] = [], referenceNetworks:Map<string,string> = new Map()): CanonicalClientResult[] {
  const map = new Map<string, CanonicalClientResult>(); const routeByCnpj=new Map(routeStores.map(store=>[store.cnpj,store]));
  transactions.forEach(tx => {
    const key=customerKey(tx); if(!key)return;
    const cnpj=validCnpj(tx.cnpj)?tx.cnpj:''; const premise=cnpj?premisesByCnpj.get(cnpj):undefined; const route=cnpj?routeByCnpj.get(cnpj):undefined;
    const resolution=resolveClientNetwork(premise?.network||'',route?.networkRaw||'',cnpj?referenceNetworks.get(cnpj)||'':'');
    if(!map.has(key))map.set(key,{cnpj,name:premise?.name||route?.name||tx.clientName,city:premise?.city||route?.city||tx.city,network:resolution.network,invoiced:0,toInvoice:0,total:0});
    const item=map.get(key)!; if(tx.status==='FATURADO')item.invoiced+=tx.value;else item.toInvoice+=tx.value;item.total=item.invoiced+item.toInvoice;
  });
...
export function buildVendorResults(transactions: SalesTransaction[], rcaByNew: Map<string,RcaMap>, rcaByOld: Map<string,RcaMap>, targets: CompassTarget[], business: {total:number;elapsed:number;remaining:number}): CanonicalVendorResult[] {
  type Temp = { invoiced:number;toInvoice:number;invClients:Set<string>;pendingClients:Set<string>;name:string;supCode:string;supName:string;rca?:RcaMap };
  const activity=new Map<string,Temp>();
  const cleanPersonName=(value:string)=>normalizeText(value).replace(/^(CLT|PJ)\s*-\s*/,'').trim();
  const resolveRca=(tx:SalesTransaction)=>{
    const direct=rcaByNew.get(tx.vendorCode)||rcaByOld.get(tx.vendorCode); if(direct)return direct;
    const activityName=cleanPersonName(tx.vendorName); if(!activityName)return undefined;
    return Array.from(rcaByNew.values()).find(item=>{const referenceName=cleanPersonName(item.name);return referenceName===activityName||referenceName.startsWith(activityName)||activityName.startsWith(referenceName)});
  };
  transactions.forEach(tx=>{
    if(!tx.vendorCode)return; const rca=resolveRca(tx); const code=rca?.newCode||tx.vendorCode;
    if(!activity.has(code))activity.set(code,{invoiced:0,toInvoice:0,invClients:new Set(),pendingClients:new Set(),name:rca?.name||tx.vendorName,supCode:rca?.coordinatorCode||tx.supervisorCode,supName:rca?.coordinatorName||tx.supervisorName,rca});
    const a=activity.get(code)!; if(!a.rca&&rca)a.rca=rca;
    if(tx.status==='FATURADO')a.invoiced+=tx.value;else a.toInvoice+=tx.value;
    if(validCnpj(tx.cnpj)){if(tx.status==='FATURADO')a.invClients.add(tx.cnpj);else a.pendingClients.add(tx.cnpj)}
  });
  const targetByOld=new Map(targets.map(t=>[t.oldCode,t])); const codes=new Set<string>(activity.keys());
  targets.forEach(target=>{const rca=rcaByOld.get(target.oldCode);codes.add(rca?.newCode||target.oldCode)});
  const rows:CanonicalVendorResult[]=[];
  codes.forEach(newCode=>{
    const a=activity.get(newCode)||{invoiced:0,toInvoice:0,invClients:new Set<string>(),pendingClients:new Set<string>(),name:'',supCode:'',supName:'',rca:rcaByNew.get(newCode)};
    const rca=a.rca||rcaByNew.get(newCode)||rcaByOld.get(newCode); const oldCode=rca?.oldCode||newCode; const target=targetByOld.get(oldCode);
...
export function buildNetworks(transactions: SalesTransaction[], premisesByCnpj: Map<string,PremiseClient>, routeStores: RouteStore[], detectedTargets: Map<string,number>, detectedOwners: Map<string,{teamCode:string;vendorCode:string}> = new Map(), clientOwners: Map<string,{teamCode:string;vendorCode:string}> = new Map(), referenceNetworks: Map<string,string> = new Map()): CanonicalNetworkResult[] {
  type VendorActivity={value:number;teamCode:string}; type Temp={name:string;invoiced:number;toInvoice:number;clients:Set<string>;vendors:Map<string,VendorActivity>}; const activity=new Map<string,Temp>(); type ClientActivity={invoiced:number;toInvoice:number}; const activityByCustomer=new Map<string,ClientActivity>(); const routeByCnpj=new Map(routeStores.map(store=>[store.cnpj,store]));
  transactions.forEach(tx=>{
    const txCustomer=customerKey(tx); const client=activityByCustomer.get(txCustomer)||{invoiced:0,toInvoice:0}; if(tx.status==='FATURADO')client.invoiced+=tx.value;else client.toInvoice+=tx.value;activityByCustomer.set(txCustomer,client);
    const cnpj=validCnpj(tx.cnpj)?tx.cnpj:''; const premise=cnpj?premisesByCnpj.get(cnpj):undefined;const route=cnpj?routeByCnpj.get(cnpj):undefined;const resolved=resolveClientNetwork(premise?.network||'',route?.networkRaw||'',cnpj?referenceNetworks.get(cnpj)||'':'');const network=resolved.network;const key=resolved.key;
    if(!activity.has(key))activity.set(key,{name:network,invoiced:0,toInvoice:0,clients:new Set(),vendors:new Map()});const a=activity.get(key)!;if(tx.status==='FATURADO')a.invoiced+=tx.value;else a.toInvoice+=tx.value;a.clients.add(txCustomer);
...
export function legacySellOut(transactions:SalesTransaction[],vendors:CanonicalVendorResult[],clients:CanonicalClientResult[]):SellOutData {
  ...
  const coordenadores=Array.from(coordMap.values()).map(c=>({...c,vendedores:c.vendedores.sort((a,b)=>b.faturado-a.faturado)})).sort((a,b)=>b.faturado-a.faturado);const invoiced=transactions.filter(t=>t.status==='FATURADO').reduce((s,t)=>s+t.value,0);const toInvoice=transactions.filter(t=>t.status==='A FATURAR').reduce((s,t)=>s+t.value,0);const invClients=new Set(transactions.filter(t=>t.status==='FATURADO'&&validCnpj(t.cnpj)).map(t=>t.cnpj));const allClients=new Set(transactions.filter(t=>validCnpj(t.cnpj)).map(t=>t.cnpj));
