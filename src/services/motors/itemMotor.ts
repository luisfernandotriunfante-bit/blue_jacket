...
function parseInternalPack(rows:Row[]):Map<string,number>{
  const map=new Map<string,number>();
  for(let i=1;i<rows.length;i++){const ean=validBarcode(rows[i][4]);const caseWeight=parseNumber(rows[i][8]);const unitWeight=parseNumber(rows[i][9]);if(!ean||caseWeight<=0||unitWeight<=0)continue;const factor=caseWeight/unitWeight;if(Number.isFinite(factor)&&factor>0)map.set(ean,Math.round(factor*1000)/1000)}
  return map;
}
