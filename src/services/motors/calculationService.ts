...
function buildVendors(unified:UnifiedDataLayer,base:CanonicalState):CanonicalVendorResult[]{
  ...
  unified.salesFacts.filter(row=>row.rcaCanonicalId).forEach(row=>{const current=salesByRca.get(row.rcaCanonicalId)||[];current.push(row);salesByRca.set(row.rcaCanonicalId,current)});
  const businessTotal=Math.max(base.sellOut.businessDaysTotal,0),elapsed=Math.max(base.sellOut.businessDaysElapsed,0),remaining=Math.max(base.sellOut.businessDaysRemaining,0);
  return unified.rcas.filter(rca=>rca.isColgate).map(rca=>{
    const rows=salesByRca.get(rca.rcaCanonicalId)||[],target=targetsByRca.get(rca.rcaCanonicalId)||{sales:0,positivity:0};
    const invoiced=sum(rows.filter(row=>row.salesStatus==='FATURADO'),row=>row.value),total=sum(rows,row=>row.value),toInvoice=total-invoiced,pos=vendorPositivity(rows);
    const idealSalesToday=businessTotal>0?target.sales*(elapsed/businessTotal):0,idealPositivityToday=businessTotal>0?target.positivity*(elapsed/businessTotal):0;
...
function buildCoordinators(vendors:CanonicalVendorResult[]):CanonicalCoordinatorResult[]{
  ...
  vendors.forEach(vendor=>{const key=vendor.coordinatorCode||vendor.coordinatorName||'SEM_COORDENADOR';const rows=grouped.get(key)||[];rows.push(vendor);grouped.set(key,rows)});
...
function buildClients(unified:UnifiedDataLayer):CanonicalClientResult[]{
  const grouped=new Map<string,SalesFactRecord[]>();unified.salesFacts.filter(row=>row.cnpj).forEach(row=>{const rows=grouped.get(row.cnpj)||[];rows.push(row);grouped.set(row.cnpj,rows)});
  const customerByCnpj=new Map(unified.customers.map(row=>[row.cnpj,row]));const latestClass=new Map<string,string>();unified.customerClassifications.forEach(row=>{if(row.premiseNetwork)latestClass.set(row.cnpj,row.premiseNetwork)});
...
function buildHistory(unified:UnifiedDataLayer,referenceDate:string):CanonicalHistorySummary{
  ...
  const currentKey=referenceDate.slice(0,7),lastYear=`${Number(currentKey.slice(0,4))-1}-${currentKey.slice(5,7)}`,averageKeys=[shiftMonth(currentKey,-3),shiftMonth(currentKey,-2),shiftMonth(currentKey,-1)];
...
function buildLines(unified:UnifiedDataLayer,sellOutTarget:number,config:ManualConfiguration):CanonicalLineResult[]{
  const itemById=new Map(unified.items.map(item=>[item.itemCanonicalId,item]));const totals=new Map<LineName,number>();
  unified.salesFacts.forEach(row=>{const item=itemById.get(row.itemCanonicalId);if(!item)return;const line=itemLine(item);if(!line)return;totals.set(line,(totals.get(line)||0)+row.value)});
  return (Object.keys(config.lineShares) as LineName[]).map(name=>{const share=config.lineShares[name]||0,target=sellOutTarget*share,total=totals.get(name)||0;const invoiced=unified.salesFacts.filter(row=>{if(row.salesStatus!=='FATURADO')return false;const item=itemById.get(row.itemCanonicalId);return Boolean(item&&itemLine(item)===name)}).reduce((s,row)=>s+row.value,0);return{name,share,target,invoiced,toInvoice:total-invoiced,total,attainment:ratio(total,target)}});
}
...
export function buildInventoryFromUnified(unified:UnifiedDataLayer,referenceDate:string,config:ManualConfiguration):{inventory:CanonicalInventoryProduct[];stock:CanonicalStockSummary}{
  const inboundByItem=new Map<string,InboundOrderFactRecord[]>();unified.inboundOrders.filter(openInbound).forEach(row=>{const rows=inboundByItem.get(row.itemCanonicalId)||[];rows.push(row);inboundByItem.set(row.itemCanonicalId,rows)});
