...
import type { CanonicalSalesTransaction } from '../../domain/canonical';
import type {
...
export function buildCustomerMaster(input:{portfolioCustomers:CustomerMasterRecord[];premiseSeeds:Partial<CustomerMasterRecord>[];topRetailers:TopRetailerSnapshotRecord[];sales:CanonicalSalesTransaction[];referenceDate:string}):CustomerMasterRecord[]{
  const map=new Map<string,CustomerMasterRecord>(); const ensure=(cnpj:string,partial:Partial<CustomerMasterRecord>)=>{if(!validCnpj(cnpj))return;const current=map.get(cnpj);const blank:CustomerMasterRecord={customerCanonicalId:customerId(cnpj),winthorCustomerCode:'',cnpj,cnpjRaw:cnpj,cnpjNormalizationStatus:'',customerName:'',tradeName:'',commercialActivity:'',city:'',district:'',address:'',latitude:null,longitude:null,buyer:'',phone:'',firstSeenAt:input.referenceDate,lastSeenAt:input.referenceDate};map.set(cnpj,{...blank,...partial,...current,...(current?{}:partial),customerCanonicalId:customerId(cnpj),cnpj})};
  input.premiseSeeds.forEach(seed=>seed.cnpj&&ensure(seed.cnpj,seed)); input.topRetailers.forEach(top=>ensure(top.cnpj,{customerName:top.storeName,tradeName:top.topTradeName})); input.sales.forEach(sale=>{if(validCnpj(sale.cnpj))ensure(sale.cnpj,{winthorCustomerCode:sale.clientCode,customerName:sale.clientName,city:sale.city})}); input.portfolioCustomers.forEach(customer=>map.set(customer.cnpj,{...(map.get(customer.cnpj)||customer),...customer})); return Array.from(map.values());
}

export function runCustomerMotor(input:{premisesRows:Row[];rcaRows:Row[];customerPortfolioRows:Row[];routeWorkbook:XLSX.WorkBook|null;routeCompetence:string;sales:CanonicalSalesTransaction[];snapshotDate:string}):CustomerMotorResult {
  ...
  const customers=buildCustomerMaster({portfolioCustomers:portfolio.customers,premiseSeeds:premises.customerSeeds,topRetailers,sales:input.sales,referenceDate:input.snapshotDate});
