...
export async function exportSellOutExcel(view: SellOutViewModel) { const template = await fetchTemplate('/templates/painel-sell-out-padrao.xlsx'); const data = fillSellOutTemplateBytes(template, view); download(new Blob([data], { type: reportMime }), `Painel Sell Out MILENIO - ${competenceName(view.competence)}.xlsx`); }
export async function exportTopNetworksExcel(view: TopNetworksViewModel) { const template = await fetchTemplate('/templates/top-redes-padrao.xlsx'); const data = fillTopNetworksTemplateBytes(template, view); download(new Blob([data], { type: reportMime }), `Top Redes MILENIO - ${competenceName(view.competence)}.xlsx`); }
