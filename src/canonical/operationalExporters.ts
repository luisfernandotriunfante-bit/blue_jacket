...
function style(sheet: XLSX.WorkSheet, headers: string[]) { headers.forEach((header, col) => { for (let row = 1; ; row += 1) { const cell = sheet[XLSX.utils.encode_cell({ r: row, c: col })]; if (!cell) break; if (['Meta', 'Faturado', 'A faturar', 'Realizado', 'Total', 'Meta da rede', 'Meta Top Varejista'].includes(header)) { cell.t = 'n'; cell.z = money; } if (['Atingimento', 'Atingimento positivação', 'Participação', 'Ating. Meta Rede', 'Ating. Meta Top'].includes(header)) { cell.t = 'n'; cell.z = pct; } if (['Clientes positivos', 'Clientes'].includes(header)) cell.t = 'n'; if (['Supervisor', 'Código supervisor', 'RCA', 'Código RCA atual', 'Código RCA antigo', 'rca_canonical_id', 'codigo_origem', 'status_relacionamento', 'Rede'].includes(header)) { cell.t = 's'; cell.v = String(cell.v ?? ''); } } }); sheet['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: 0, c: headers.length - 1 } }) }; }
