...
export type ActiveCanonicalBundle={
  ...
  /** Obrigatório para builds v19; combina fontes físicas + Admin Registry. */
...
function isActiveCanonicalBundle(value:unknown):value is ActiveCanonicalBundle{if(!value||typeof value!=='object')return false;const candidate=value as Partial<ActiveCanonicalBundle>;if(candidate.status!=='ACTIVE'||typeof candidate.motorBuildId!=='string'||!candidate.motorBuildId||typeof candidate.stagingManifestHash!=='string'||!candidate.stagingManifestHash||typeof candidate.schemaVersion!=='string'||typeof candidate.engineVersion!=='string'||!candidate.rowCounts||!candidate.factTypeCounts)return false;if(candidate.engineVersion==='browser-stage4-product-assortment-v19-admin-registry-authority'&&(!candidate.adminRegistryHash||!candidate.canonicalInputHash))return false;return true}

/** Resolves active v19 builds and recognizes older active pointers only so sourceImport can rebuild them safely. */
