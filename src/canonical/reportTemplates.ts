...
function sellOutCells(view: SellOutViewModel): Record<string, CellValue> {
  const topLines = view.salesByLine.slice(0, 5);
  ...
  for (let day = 1; day <= 31; day += 1) { const date = `${view.competence}-${String(day).padStart(2, '0')}`; const row = view.dailyRows.find(item => item.date === date); const target = 7 + day; cells[`C${target}`] = Math.floor(Date.parse(`${date}T00:00:00Z`) / 86400000) + 25569; cells[`E${target}`] = row?.realized ?? 0; cells[`F${target}`] = row?.invoiced ?? 0; cells[`G${target}`] = row?.totalPositivation ?? 0; }
...
export function fillSellOutTemplateBytes(template: Uint8Array, view: SellOutViewModel & TargetProvenance) { return workbookBytes(template, ['SELL OUT - Milenio 2026', 'EQUIPES'], entries => { patch(entries, 'SELL OUT - Milenio 2026', xml => setCells(xml, sellOutCells(view))); const teamRows = view.vendorRows.map(row => [row.label, row.salesTarget, row.invoiced, row.toInvoice, row.realized, row.positiveCustomers, row.achievement]); patch(entries, 'EQUIPES', xml => replaceDataRows(xml, 3, [['RCA', 'META', 'FATURADO', 'A FATURAR', 'SELL OUT', 'POSITIVAÇÃO', 'ATINGIMENTO'], ...teamRows])); updateCoreProperties(entries, 'Painel Sell Out MILENIO', `Snapshot estático do build ${view.motorBuildId}; competência ${view.competence}; targetStateHash ${view.targetStateHash ?? 'indisponível'}`); }); }
...
export function fillTopNetworksTemplateBytes(template: Uint8Array, view: TopNetworksViewModel & TargetProvenance) { return workbookBytes(template, ['Top Redes'], entries => { patch(entries, 'Top Redes', xml => { let next = replaceDataRows(xml, 3, [['REDE', 'EQUIPE', 'CÓD RCA', 'META REDES', 'META TOPS', 'FATURADO R$', '% FAT. REDES', '% FAT. TOPS', 'A FATURAR R$', 'GAP R$', '% FAT. + A FATURAR REDES', '% FAT. + A FATURAR TOPS', 'ATUALIZADO'], ...topRows(view)]); return setCells(next, { B2: view.competence, D2: view.totals.networkTarget === null ? 'Não configurada' : view.totals.networkTarget, E2: view.rows.reduce((sum, row) => sum + (row.topTarget ?? 0), 0), F2: view.totals.invoiced, I2: view.totals.toInvoice, M2: view.generatedAt }); }); updateCoreProperties(entries, 'Top Redes MILENIO', `Snapshot estático do build ${view.motorBuildId}; competência ${view.competence}; targetStateHash ${view.targetStateHash ?? 'indisponível'}`); }); }
