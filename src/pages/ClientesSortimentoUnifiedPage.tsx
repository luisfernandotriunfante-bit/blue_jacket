...
export function ClientesSortimentoPage({ view = 'overview' }: { view?: ClientesSortimentoView }) {
  ...
  const result = useMemo(() => unifiedCanonical && selectedCnpj ? buildCustomerIntelligence(unifiedCanonical, support, selectedCnpj, unifiedCanonical.referenceDate) : null, [unifiedCanonical, support, selectedCnpj]);
  ...
  const selector = <PanelCard><PanelSectionHeader eyebrow="CLIENTE" title="Inteligência Comercial por CNPJ" description="A análise operacional fica presa à fotografia canônica atual para não misturar sortimento antigo com perfil, estoque, Carteira e preço atuais. O histórico financeiro permanece visível separadamente." action={<div style={{ display: 'grid', gap: 4 }}><span className="panel-mini-label">Referência operacional</span><strong>{fmtDate(unifiedCanonical.referenceDate)}</strong></div>} /><div className="panel-grid panel-grid-2" style={{ marginTop: 14 }}><input className="panel-input panel-input-full" placeholder="Buscar CNPJ, cliente, rede, cidade ou faixa" value={search} onChange={event => setSearch(event.target.value)} /><select className="panel-select panel-input-full" value={selectedCnpj} onChange={event => setSelectedCnpj(event.target.value)}><option value="">Selecione um CNPJ</option>{filteredCustomers.map(item => <option key={item.cnpj} value={item.cnpj}>{item.cnpj} · {item.name || 'Cliente sem nome'} · {item.network || 'Sem rede'} · {item.tier || 'Sem faixa'}</option>)}</select></div></PanelCard>;
  ...
  const blockedAudits = result.audit.filter(check => check.status === 'BLOCKED');
  ...
  const overview = <div className="panel-stack">
    ...
    {(divergentAudits.length || blockedAudits.length || result.limitations.length) ? <PanelCard><PanelSectionHeader eyebrow="AUDITORIA DA FICHA" title={`${divergentAudits.length} divergência(s) · ${blockedAudits.length} bloqueio(s) · ${result.limitations.length} limitação(ões)`} description="Checks não aprovados ficam explícitos; bloqueios não são convertidos em cálculo válido." />{divergentAudits.map(check => <div key={check.id} className="panel-muted" style={{ marginTop: 8 }}><strong>{check.label}</strong> · DIVERGENTE · {check.note || `${check.expected} × ${check.calculated}`}</div>)}{blockedAudits.map(check => <div key={check.id} className="panel-muted" style={{ marginTop: 8 }}><strong>{check.label}</strong> · BLOQUEADO · {check.note || 'Sem evidência suficiente para validar este check.'}</div>)}{result.limitations.map((limit, index) => <div key={`limit-${index}`} className="panel-muted" style={{ marginTop: 8 }}>• {limit}</div>)}</PanelCard> : null}
  </div>;
