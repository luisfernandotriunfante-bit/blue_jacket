...
import { PanelCard, PanelEmptyState, PanelKpi, PanelPage, PanelSectionHeader, PanelStat } from '../ui/pattern/PanelVisual';
...
export function ClientesSortimentoPage({ view = 'overview' }: { view?: ClientesSortimentoView }) {
  ...
  if (!canonical || !isUnifiedCanonicalState(canonical)) return <PanelPage title="Clientes & Sortimento"><PanelEmptyState variant="page" title="Base canônica unificada ainda não processada" description="Atualize as fontes em Configurações. Este módulo não possui mais upload ou cálculo paralelo." /></PanelPage>;
  ...
  const promotions = <PanelCard><PanelSectionHeader eyebrow="PROMOÇÕES" title={support.promotions.length ? `${result.promotions.length} promoções elegíveis` : 'Fonte estruturada pendente'} />{support.promotions.length ? <div className="panel-grid panel-grid-auto" style={{marginTop:14}}>{result.promotions.map(rule=><PanelStat key={rule.id} label={rule.name} value={rule.benefit||'Benefício não informado'} />)}</div> : <PanelEmptyState variant="section" title="Nenhuma promoção oficial estruturada" description="Nada é inventado sem fonte validada." />}</PanelCard>;
