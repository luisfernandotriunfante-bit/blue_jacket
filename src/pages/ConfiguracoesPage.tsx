...
import { PanelAlert, PanelCard, PanelPage, PanelSectionHeader, PanelSectionNav } from '../ui/pattern/PanelVisual';
...
export function ConfiguracoesPage() {
  ...
  return <PanelPage title="Configurações" metricLabel="Fontes registradas" metricValue={`${loadedCount}/${SOURCES.length}`}>
    ...
    <PanelSectionNav items={[{ id: 'config-atualizacao', label: 'Atualização' }, { id: 'config-continuidade', label: 'Continuidade' }, { id: 'config-fontes', label: 'Fontes' }, { id: 'config-auditoria', label: 'Auditoria' }]} />

    <div id="config-atualizacao"><PanelCard>
    ...
    <div id="config-continuidade">{portfolioContinuity ? <PortfolioContinuityPanel snapshot={portfolioContinuity} /> : null}<FrozenSnapshotsPanel snapshots={frozenSnapshots} />{operationalState.receiptItems.length > 0 ? <ReceiptValidationPanel key={`${operationalState.entry218FileName}:${operationalRevision}`} state={operationalState} /> : null}</div>

    <div id="config-fontes">{GROUPS.map(group => <PanelCard key={group.key}><PanelSectionHeader eyebrow={group.key.toUpperCase()} title={group.title} description={group.description} action={<span className="panel-badge">{SOURCES.filter(source => source.group === group.key).length} arquivos</span>} /><div style={{ display: 'grid', gap: '8px', marginTop: '15px' }}>{SOURCES.filter(source => source.group === group.key).map(source => <SourceCard key={source.id} source={source} audit={source.kind ? audits.get(source.kind) : undefined} operationalState={operationalState} unifiedLoaded={isUnifiedLoaded(source)} queued={queued.get(source.id)} onAddFile={file => addFiles([file])} />)}</div></PanelCard>)}</div>

    <div id="config-auditoria">{canonical ? <ReconciliationAuditPanel checks={canonical.reconciliation?.checks || []} /> : null}
