...
import { applyOperationalOverrides, operationalLegacyData, prepareOperationalSources, supplementalSourceLabel } from '../services/operationalSources';
...
const SOURCES:SourceUi[]=[
 ...
 {kind:'stock105',label:'Posição Estoque 105',description:'Estoque físico/financeiro e custo. O preço de tabela pode ser sobrescrito pela PCTABPR.',group:'Rotina diária',required:true},
 {kind:'purchasePortfolio',label:'Carteira Colgate',description:'Estoque em trânsito. Quando a Carteira expõe NF/Invoice, notas já recebidas são abatidas.',group:'Rotina diária',required:true},
 ...
 {kind:'priceList',label:'Lista de Preço Colgate',description:'Referência Colgate → Milênio, EANs, Un/CX e classificação. Não substitui a PCTABPR.',group:'Cadastros de apoio'},
...
export function ConfiguracoesPage(){
 ...
 const handleProcess=async()=>{if(!selectedFiles.length)return;setIsProcessing(true);setSuccess(false);setErrorMessage('');try{const prepared=await prepareOperationalSources(selectedFiles);const result=await processCanonicalFiles(prepared.engineFiles,manualConfig,canonical);const adjusted=applyOperationalOverrides(result.canonical,prepared.state,manualConfig);const legacy=operationalLegacyData(adjusted.canonical,manualConfig.coverageTargetDays);setCanonical(adjusted.canonical);setProdutos(legacy.produtos);setMetricas(legacy.metricas);setSellOut(result.sellOut);setSuccess(true);setSelectedFiles([])}catch(error){console.error(error);setErrorMessage(error instanceof Error?error.message:'Não foi possível processar os arquivos.')}finally{setIsProcessing(false)}};
 ...
 return <PanelPage title="Configurações" metricLabel="Fontes registradas" metricValue={`${SOURCES.filter(source=>audits.get(source.kind)?.loaded).length}/${SOURCES.length}`}>
  <PanelCard>
   <PanelSectionHeader eyebrow="ATUALIZAÇÃO" title="Adicionar ou atualizar arquivos" description="Você pode enviar apenas o que mudou. PCTABPR, Entrada 218 e 12.322 também são reconhecidos e permanecem como fontes operacionais para preço e recebimento." action={<span className="panel-badge">Aceita Excel e TXT histórico</span>}/>
