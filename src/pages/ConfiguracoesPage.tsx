...
export function ConfiguracoesPage() {
  ...
  return <PanelPage title="Configurações" metricLabel="Fontes registradas" metricValue={`${loadedCount}/${SOURCES.length}`}>
    ...
    {activeView === 'continuity' ? <div id="config-continuidade">{portfolioContinuity ? <PortfolioContinuityPanel snapshot={portfolioContinuity} /> : null}<FrozenSnapshotsPanel snapshots={frozenSnapshots} />{operationalState.receiptItems.length > 0 ? <ReceiptValidationPanel key={`${operationalState.entry218FileName}:${operationalRevision}`} state={operationalState} /> : null}</div> : null}
    ...
    {activeView === 'audit' ? <div id="config-auditoria">{canonical ? <ReconciliationAuditPanel checks={canonical.reconciliation?.checks || []} /> : null}
