...
export function ConfiguracoesPage() {
  ...
  const [activeView, setActiveView] = useState<'update' | 'continuity' | 'audit'>('update');
  ...
  return <PanelPage title="Configurações" metricLabel="Fontes registradas" metricValue={`${loadedCount}/${SOURCES.length}`}>
    ...
    <PanelTabs tabs={[{ id: 'update', label: 'Atualizar lote' }, { id: 'continuity', label: 'Continuidade' }, { id: 'audit', label: 'Auditoria e pendências' }]} activeId={activeView} onChange={setActiveView} ariaLabel="Áreas de Configurações" />
    ...
    </PanelCard>{operationalState.receiptItems.length > 0 ? <ReceiptValidationPanel key={`${operationalState.entry218FileName}:${operationalRevision}`} state={operationalState} /> : null}<div className="panel-stack" style={{ marginTop: 20 }}>{GROUPS.map(group => <PanelDisclosure key={group.key} eyebrow={group.key.toUpperCase()} title={group.title} description={group.description} action={<span className="panel-badge">{SOURCES.filter(source => source.group === group.key).length} arquivos</span>} defaultOpen={group.key === 'Rotina diária'}><div style={{ display: 'grid', gap: '8px' }}>{SOURCES.filter(source => source.group === group.key).map(source => <SourceCard key={source.id} source={source} audit={source.kind ? audits.get(source.kind) : undefined} operationalState={operationalState} unifiedLoaded={isUnifiedLoaded(source)} queued={queued.get(source.id)} onAddFile={file => addFiles([file])} />)}</div></PanelDisclosure>)}</div></div> : null}
