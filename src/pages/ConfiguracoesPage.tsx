...
export function ConfiguracoesPage() {
  ...
  const [activeView, setActiveView] = useState<'update' | 'continuity' | 'sources' | 'audit'>('update');
  ...
  const auditNotices = useMemo(() => {
    const notices = (canonical?.warnings || []).map(warning => ({ severity: 'WARNING', text: warning }));
    SOURCES.filter(source => source.required && source.kind && !audits.get(source.kind)?.loaded).forEach(source => {
      notices.push({ severity: 'BLOCKED', text: `${source.label}: fonte principal ainda não carregada.` });
    });
    return notices;
  }, [canonical, audits]);
  ...
  return <PanelPage title="Configurações" metricLabel="Fontes registradas" metricValue={`${loadedCount}/${SOURCES.length}`}>
    ...
    <PanelTabs tabs={[{ id: 'update', label: 'Atualizar lote' }, { id: 'continuity', label: 'Continuidade' }, { id: 'sources', label: 'Fontes cadastradas' }, { id: 'audit', label: 'Auditoria e pendências' }]} activeId={activeView} onChange={setActiveView} ariaLabel="Áreas de Configurações" />

    {activeView === 'update' ? <div id="config-atualizacao"><PanelCard>
    ...
    </PanelCard>{operationalState.receiptItems.length > 0 ? <ReceiptValidationPanel key={`${operationalState.entry218FileName}:${operationalRevision}`} state={operationalState} /> : null}</div> : null}
    ...
    {activeView === 'sources' ? <div id="config-fontes" className="panel-stack">{GROUPS.map(group => <PanelDisclosure key={group.key} eyebrow={group.key.toUpperCase()} title={group.title} description={group.description} action={<span className="panel-badge">{SOURCES.filter(source => source.group === group.key).length} arquivos</span>} defaultOpen={group.key === 'Rotina diária'}><div style={{ display: 'grid', gap: '8px' }}>{SOURCES.filter(source => source.group === group.key).map(source => <SourceCard key={source.id} source={source} audit={source.kind ? audits.get(source.kind) : undefined} operationalState={operationalState} unifiedLoaded={isUnifiedLoaded(source)} queued={queued.get(source.id)} onAddFile={file => addFiles([file])} />)}</div></PanelDisclosure>)}</div> : null}
    ...
    <PanelCard><PanelSectionHeader eyebrow="AVISOS CONSOLIDADOS" title="Central de avisos do sistema" description="Pendências de fontes, persistência e regras que precisam de atenção ficam reunidas nesta área." action={<span className="panel-badge">{auditNotices.length} aviso(s)</span>} />{auditNotices.length ? <div style={{ display: 'grid', gap: '8px', marginTop: '14px' }}>{auditNotices.map((notice, index) => <div key={`${notice.text}-${index}`} className={`panel-alert panel-alert-${notice.severity === 'BLOCKED' ? 'warning' : 'info'}`}><strong>{notice.severity === 'BLOCKED' ? 'BLOQUEADO' : 'ATENÇÃO'}</strong><span>{notice.text}</span></div>)}</div> : <PanelAlert tone="success">Nenhum aviso pendente na fotografia atual.</PanelAlert>}</PanelCard>
