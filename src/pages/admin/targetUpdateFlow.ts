...
import { rcaTargetRegistryHash, targetStateHash } from '../../canonical/targetIdentity';
...
export type TargetUpdateCoordinatorState = { phase: TargetUpdatePhase; busy: boolean; status: string; error: string; motorBuildId: string | null; targetStateHash: string | null; rcaTargetRegistryHash: string | null; canonicalInputHash: string | null };
type ActivePhase = 'MUTATING' | 'BUILDING' | 'ACTIVATING' | 'SYNCING';
export type TargetUpdateControls = { setPhase: (phase: ActivePhase) => void };
export type TargetUpdateCompletion = { phase: 'SUCCESS' | 'LOCAL_SUCCESS_SYNC_FAILED' | 'FAILED'; status: string; error: string; motorBuildId?: string | null; targetStateHash?: string | null; rcaTargetRegistryHash?: string | null; canonicalInputHash?: string | null };
...
export function createTargetUpdateCoordinator() {
  let state = idle(); const listeners = new Set<Listener>(); const publish = (patch: Partial<TargetUpdateCoordinatorState>) => { state = { ...state, ...patch }; for (const listener of listeners) listener(state); };
  return {
    getState: () => state,
    ...
    async run(operation: (controls: TargetUpdateControls) => Promise<TargetUpdateCompletion>) {
      ...
      try { const completion = await operation({ setPhase: phase => publish({ phase }) }); publish({ phase: completion.phase, busy: false, status: completion.status, error: completion.error, motorBuildId: completion.motorBuildId ?? null, targetStateHash: completion.targetStateHash ?? null, rcaTargetRegistryHash: completion.rcaTargetRegistryHash ?? null, canonicalInputHash: completion.canonicalInputHash ?? null }); return { status: 'DONE', value: completion } as const; }
      catch (reason) { const error = reason instanceof Error ? reason.message : String(reason); const completion: TargetUpdateCompletion = { phase: 'FAILED', status: '', error }; publish({ ...completion, busy: false, motorBuildId: null, targetStateHash: null, rcaTargetRegistryHash: null, canonicalInputHash: null }); return { status: 'DONE', value: completion } as const; }
    },
  };
}
export const targetUpdateCoordinator = createTargetUpdateCoordinator();
export type TargetUpdateRuntime = { getActive: () => ActiveCanonicalBundle | null; activate: (active: ActiveCanonicalBundle) => void; deactivate: () => void; autoSync?: (active: ActiveCanonicalBundle) => Promise<BaseAutoSyncResult> };
...
export function assertTargetCompetenceOpen(competence: string, state: CompetenceState | null) { if (competenceRecord(state, competence)?.status !== 'OPEN') throw new Error('TARGET_COMPETENCE_NOT_OPEN'); }

export async function executeTargetUpdateTransaction(competence: string, mutation: (current: TargetState | null) => Promise<TargetState> | TargetState, runtime: TargetUpdateRuntime, controls: TargetUpdateControls, dependencies: TargetTransactionDependencies = defaults): Promise<TargetUpdateCompletion> {
  const previousTarget = dependencies.loadTarget(); const previousActive = runtime.getActive(); const previousRcaHash = await dependencies.rcaHash(previousTarget); let activated = false;
  try {
    controls.setPhase('MUTATING'); const next = await mutation(previousTarget ? structuredClone(previousTarget) : null);
    assertTargetCompetenceOpen(competence, dependencies.loadCompetence()); dependencies.replaceTarget(next);
    const verified = dependencies.loadTarget(); const fullHash = await dependencies.targetHash(verified); const nextRcaHash = await dependencies.rcaHash(verified); const requiresRebuild = previousRcaHash !== nextRcaHash;
    let active = runtime.getActive();
    if (requiresRebuild) {
      controls.setPhase('BUILDING'); const rebuilt = await dependencies.build(verified);
      if (!rebuilt.adminRegistryHash || rebuilt.engineVersion !== dependencies.engineVersion || rebuilt.rcaTargetRegistryHash !== nextRcaHash || rebuilt.sourceContractVersion !== 'v2' || !rebuilt.sourceReplacementProofHash) throw new Error('TARGET_BUILD_IDENTITY_MISMATCH');
      const expectedInput = await dependencies.inputHash(rebuilt.stagingManifestHash, rebuilt.adminRegistryHash, nextRcaHash, rebuilt.sourceReplacementProofHash);
      if (rebuilt.canonicalInputHash !== expectedInput) throw new Error('TARGET_BUILD_IDENTITY_MISMATCH');
      controls.setPhase('ACTIVATING'); runtime.activate(rebuilt); activated = true; active = rebuilt;
    }
    if (active) {
      controls.setPhase('SYNCING'); let sync: BaseAutoSyncResult | { status: 'SYNC_FAILED'; error: unknown };
      try { sync = runtime.autoSync ? await runtime.autoSync(active) : await dependencies.sync(active); } catch (error) { sync = { status: 'SYNC_FAILED', error }; }
    ...
    if (!activated) { try { dependencies.replaceTarget(previousTarget); if (previousActive) runtime.activate(previousActive); else runtime.deactivate(); } catch { /* original */ } }
    throw reason;
  }
}

export function runCanonicalTargetMutation(competence: string, mutation: (current: TargetState | null) => Promise<TargetState> | TargetState, runtime: TargetUpdateRuntime) { return systemDataOperationCoordinator.run('TARGET_UPDATE', async () => targetUpdateCoordinator.run(controls => executeTargetUpdateTransaction(competence, mutation, runtime, controls))); }
...
export async function migrateLegacyGeneralTarget(kind: 'sellOut' | 'positivity', competence: string, runtime: TargetUpdateRuntime) {
  const settingsBefore = loadReportSettings(); const value = kind === 'sellOut' ? settingsBefore.legacySellOutTarget : settingsBefore.legacyPositivityTarget;
  ...
  if (result.status === 'DONE' && result.value.status === 'DONE' && result.value.value.phase !== 'FAILED') { const next = loadReportSettings(); if (kind === 'sellOut') next.legacySellOutTarget = null; else next.legacyPositivityTarget = null; restoreReportSettings(next); }
  return result;
}