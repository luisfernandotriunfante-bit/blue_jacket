...
import { buildStockPresentation, isOperationalNoWinthor } from '../domain/stockModel';
...
export function LancamentosPage() {
  ...
  // Todos os hooks são executados mesmo durante hidratação. Isso evita mudança de ordem quando canonical passa de null para carregado.
  ...
  }), [canonical, inventory, unified, hasStock105]);
  ...
  const totals = useMemo(() => launches.reduce((acc, product) => {
    ...
    // Valor pendente a venda só pode ser unidades comprovadas × PVENDA1; sem ambos, fica zero conhecido e a pendência é sinalizada.
    const pendingSale = product.pendingUnits > 0 && product.saleUnit > 0 ? product.pendingUnits * product.saleUnit : 0;
  ...
  const unvaluedPortfolio = launches.filter(product => (product.pendingUnits > 0 || product.pendingCases > 0) && (product.pendingUnits <= 0 || product.saleUnit <= 0)).length;
  ...
  return (
    <PanelPage title="Lançamentos" metricLabel="Potencial projetado conhecido" metricValue={formatCurrency(totals.projectedSale)}>
      <div className="panel-stack">
        <div className="panel-grid panel-grid-4">
          ...
          <PanelKpi label="Na Carteira" value={formatNumber(inPortfolio)} detail={unvaluedPortfolio ? `${formatNumber(unvaluedPortfolio)} sem valorização completa por falta de Un/CX e/ou PVENDA1` : 'Entrada prevista valorizada por PVENDA1'} tone="blue" />
          <PanelKpi label="Potencial projetado" value={formatCurrency(totals.projectedSale)} detail={`Após reserva + Carteira valorizável · Custo proj.: ${formatCurrency(totals.projectedCost)}`} tone="red" />
        </div>
        ...
        <PanelCard>
          <PanelSectionHeader
            ...
            description="Lançamento é definido exclusivamente pela Lista Oficial por EAN. Custo só existe com 105 e preço de venda só existe com PVENDA1 Região 11."
          ...
          <div className="panel-table-wrap stock-table-compact"><table className="panel-table">
            ...
            <tbody>{filtered.length ? filtered.map(product => <tr key={`${product.ean}-${product.code}`}>
              ...
              <td><div className="panel-badges"><span className="panel-badge panel-badge-purple">LANÇAMENTO</span>{product.physicalTotalUnits > 0 ? <span className="panel-badge panel-badge-green">COM ESTOQUE</span> : <span className="panel-badge panel-badge-red">SEM ESTOQUE</span>}{(product.pendingUnits > 0 || product.pendingCases > 0) && <span className="panel-badge panel-badge-blue">EM CARTEIRA</span>}{isOperationalNoWinthor(product) && <span className="panel-badge panel-badge-amber">SEM WINTHOR</span>}</div></td>
