...
export type ProductCatalogItem = {
  ...
  sold: number; salesValue: number; lastSale: string | null; coverage: number | null; isLaunch: boolean; unregistered: boolean;
};
...
export function buildProductCatalog(m1: CanonicalList, m3: CanonicalList): ProductCatalogItem[] {
  ...
  for (const raw of m1.records as Row[]) {
    ...
    add({ id, description: first(raw, ['description_internal', 'description_industry']) || 'Produto sem descrição', winthor, distributor, ean: first(raw, ['internal_ean', 'industry_ean']), physical: num(raw.physical_stock_units), reserved: num(raw.stock_286_reserved), available: num(raw.stock_286_available) || Math.max(0, num(raw.physical_stock_units) - num(raw.stock_286_reserved)), price: Number.isFinite(Number(raw.pVenda1_region11)) && Number(raw.pVenda1_region11) > 0 ? Number(raw.pVenda1_region11) : null, inboundValue: 0, inboundQty: 0, invoices: [], arrival: null, lastReceipt: null, sold: 0, salesValue: 0, lastSale: null, coverage: null, isLaunch: raw.is_launch === true, unregistered: false });
  }
  ...
  for (const raw of m3.records as Row[]) {
    ...
    if (!item && type === 'INBOUND_ORDER') {
      ...
      item = items.get(id) ?? add({ id, description: `Item da Carteira — ${distributor || 'sem código'}`, winthor: '', distributor, ean: '', physical: 0, reserved: 0, available: 0, price: null, inboundValue: 0, inboundQty: 0, invoices: [], arrival: null, lastReceipt: null, sold: 0, salesValue: 0, lastSale: null, coverage: null, isLaunch: false, unregistered: true });
    }
    if (!item) continue;
  ...
  const days = 90;
  return [...items.values()].map(item => ({ ...item, coverage: item.sold > 0 ? item.available / (item.sold / days) : null })).sort((a,b) => a.description.localeCompare(b.description));
}
...
export function ProductCatalogPage({ m1, m3, launchesOnly = false }: { m1: CanonicalList; m3: CanonicalList; launchesOnly?: boolean }) {
  ...
  return <PanelPage title={title} metricLabel="ITENS ENCONTRADOS" metricValue={qty.format(rows.length)}><div className="panel-stack product-catalog-stack">
    <PanelCard compact><PanelSectionHeader eyebrow="CATÁLOGO OPERACIONAL" title={launchesOnly ? 'Itens marcados como lançamento' : 'Todos os itens disponíveis e em Carteira'} description="Pesquise por descrição, EAN, código Winthor ou código do distribuidor. Preço é a tabela PVENDA1; promoção não é inferida sem fonte própria." />
    ...
    {rows.length ? <div className="product-list">{rows.map(item => <article className="product-row" key={item.id}>
      <button className="product-row-main" type="button" onClick={()=>setOpen(open === item.id ? null : item.id)} aria-expanded={open === item.id}>
        <div className="product-name"><strong>{item.description}</strong><span>{item.unregistered ? 'Aguarda cadastro · vindo da Carteira' : item.isLaunch ? 'Lançamento' : 'Item cadastrado'}</span><small>Winthor {item.winthor || '—'} · Distribuidor {item.distributor || '—'} · EAN {item.ean || '—'}</small></div>
        <Cell label="Disponível">{qty.format(item.available)} un.</Cell><Cell label="Carteira">{item.inboundValue ? money.format(item.inboundValue) : '—'}</Cell><Cell label="Próx. chegada">{date(item.arrival)}</Cell><Cell label="Tabela">{item.price === null ? '—' : money.format(item.price)}</Cell><Cell label="Cobertura">{item.coverage === null ? '—' : `${Math.round(item.coverage)} dias`}</Cell><span className="product-chevron">⌄</span>
      </button>
      {open === item.id ? <div className="product-details"><PanelInfoRow label="Estoque físico / reservado" value={`${qty.format(item.physical)} / ${qty.format(item.reserved)} un.`} /><PanelInfoRow label="Giro (últimos 90 dias)" value={item.sold ? `${qty.format(item.sold)} un. · ${money.format(item.salesValue)} · última venda ${date(item.lastSale)}` : 'Sem venda positiva no período'} /><PanelInfoRow label="Última entrada registrada" value={date(item.lastReceipt)} /><PanelInfoRow label="Carteira" value={item.inboundValue ? `${money.format(item.inboundValue)} · ${qty.format(item.inboundQty)} un. · NF ${item.invoices.join(', ')}` : 'Sem pedido aberto'} /><PanelInfoRow label="Atividade" value={item.isLaunch ? 'Lançamento importado' : 'Nenhuma atividade/promoção foi importada'} /></div> : null}
    </article>)}</div> : <PanelEmptyState title="Nenhum item neste filtro" description="Ajuste a busca ou a situação para ver os itens da fotografia ativa." />}
  </div></PanelPage>;
}
