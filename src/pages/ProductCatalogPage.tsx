...
import { parseAssortmentPresence, type AssortmentPresence } from '../canonical/assortment';
...
function ProductAssortment({ item }: { item: ProductCatalogItem }) { return <section className="product-assortment"><div><span>SORTIMENTO OFICIAL</span><strong>Onde este item deve aparecer</strong><small>A faixa cadastrada no cliente define a disponibilidade para o vendedor.</small></div>{item.assortment.length ? <div className="product-assortment-chips">{item.assortment.map(channel=><span key={channel.field}><strong>{channel.label}</strong>{channel.range ? <small>{channel.range}</small> : null}<em>{channel.classification}</em></span>)}</div> : <p>{item.assortmentMaterialized ? 'Item não recomendado nas faixas oficiais.' : 'Sortimento ainda não materializado para este item.'}</p>}</section>; }
