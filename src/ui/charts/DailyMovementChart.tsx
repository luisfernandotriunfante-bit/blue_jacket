    ...
    <div>
      ...
      <div className="chart-canvas-wrap">
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Gráfico diário de faturado, a faturar e Sell Out" className="chart-svg">
          ...
          {data.map((item, index) => <g key={item.date}><circle cx={x(index)} cy={y(item.total)} r="3.4" fill="var(--panel-red)" stroke="var(--panel-bg)" strokeWidth="1.5" />{(index % labelStep === 0 || index === data.length - 1) ? <text x={x(index)} y={height - 18} textAnchor="middle" fill="var(--panel-muted)" fontSize="10">{new Date(`${item.date}T12:00:00`).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</text> : null}</g>)}
        </svg>
      </div>
    </div>
  );
