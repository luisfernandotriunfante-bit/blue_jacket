      ...
      <div className="chart-canvas-wrap">
        <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Movimentação diária de positivação" className="chart-svg">
          ...
          {grid.map(level => {
            ...
            const gy = y(value);
            return <g key={level}><line x1={pad.left} y1={gy} x2={width - pad.right} y2={gy} stroke="rgba(255,255,255,0.07)" strokeWidth="1" /><text x={pad.left - 9} y={gy + 4} textAnchor="end" fill="var(--panel-muted)" fontSize="9">{Math.round(value)}</text></g>;
          })}
          ...
          {data.map((item, index) => <g key={item.date}><circle cx={x(index)} cy={y(item.totalPositivation)} r="3.2" fill="var(--panel-purple)" stroke="var(--panel-bg)" strokeWidth="1.3" /><text x={x(index)} y={height - 13} textAnchor="middle" fill="var(--panel-muted)" fontSize="9">{new Date(`${item.date}T12:00:00`).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}</text></g>)}
        </svg>
      </div>
    </div>
  );
...
function Legend({ color, label }: { color: string; label: string }) {
  return <span className="chart-legend"><span className="chart-legend-line" style={{ borderTopColor: color }} />{label}</span>;
}
