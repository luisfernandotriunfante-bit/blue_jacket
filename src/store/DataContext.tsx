...
export function DataProvider({children}:{children:ReactNode}){const [activeCanonical,setActiveCanonical]=useState<ActiveCanonicalBundle|null>(()=>resolveActiveCanonicalBundle());const activateCanonical=(bundle?:ActiveCanonicalBundle)=>setActiveCanonical(bundle?activateCanonicalBundleReference(bundle):activateApprovedCanonicalBundle());const rollback=()=>{deactivateCanonicalBundle();setActiveCanonical(null)};return <DataContext.Provider value={{activeCanonical,activateCanonical,deactivateCanonical:rollback,dataNotice:activeCanonical?`Build canônico ativo: ${activeCanonical.motorBuildId}.`:RESET_NOTICE}}>{children}</DataContext.Provider>}
