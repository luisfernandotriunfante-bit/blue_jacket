...
function workbookFile(rows:any[][],name:string):File { const workbook=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook,XLSX.utils.aoa_to_sheet(rows),'Plan1'); const bytes=XLSX.write(workbook,{type:'array',bookType:'xlsx'}); return new File([bytes],name); }
function emptyPriceList(){return{bySku:new Map(),byEan:new Map()}}

test('Posição 105 completa deriva Un/CX por Qt.Est dividido por Master',()=>{
  const rows:any[][]=[['CODIGO','DESCRICAO','ESTOQUE','MASTER','CUSTO UNITARIO','P VENDA']];
  for(let index=1;index<=50;index+=1)rows.push([index,`Produto ${index}`,120,10,2,3]);
  const products=parseStock105(rows,{byInternal:new Map(),factoryToInternal:new Map()});
  assert.equal(products.size,50); assert.equal(products.get('1')?.quantidade,120); assert.equal(products.get('1')?.vendaUnitario,3); assert.equal(products.get('1')?.unitsPerCase,12); assert.equal(products.get('1')?.unitsPerCaseSource,'105_DERIVED');
});

test('Posição 105 compacta é normalizada e deriva fator sem tratar Master como Un/CX',async()=>{
  const rows:any[][]=[];
  for(let index=1;index<=50;index+=1){const row=Array(16).fill('');row[0]=index;row[1]=`Produto ${index}`;row[4]=120;row[5]=10;row[6]=2;row[9]=3;rows.push(row)}
  ...
  assert.equal(products.size,50); assert.equal(products.get('50')?.custoUnitario,2); assert.equal(products.get('50')?.unitsPerCase,12); assert.notEqual(products.get('50')?.unitsPerCase,10);
});

test('Cadastro 286 completo reconhece Winthor, fábrica e EAN sem transformar Master em Un/CX',()=>{
...
test('Cadastro 286 não interpreta código Master 200 como fator Un/CX',()=>{
  const ean=gtin13('789123456787');const row=Array(26).fill('');row[0]='11';row[1]='101';row[2]='Produto';row[20]=ean;row[23]='MATX';row[24]='200';
  const cadastro=parseCadastro286([row]);
  assert.equal(cadastro.byInternal.get('101')?.unitsPerCase,undefined);
});

test('Cadastro 286 compacto preserva EAN e fábrica, mas Master continua sendo código cadastral',async()=>{
  const ean=gtin13('789123456788');const row=Array(21).fill('');row[0]='11';row[1]='200';row[2]='Compacto';row[17]=ean;row[18]='MAT2';row[19]='200';row[20]='S';
  ...
  assert.equal(cadastro.byInternal.get('200')?.ean,ean); assert.equal(cadastro.factoryToInternal.get('MAT2'),'200'); assert.equal(cadastro.byInternal.get('200')?.unitsPerCase,undefined);
});
...
test('Premissas usa a coluna TIPO para distinguir CNPJ com zeros perdidos de CPF/código inválido',()=>{
  ...
  assert.equal(parsed[0].cnpj,'00073351000122'); assert.equal(parsed[0].cnpjNormalizationStatus,'PADDED_EXCEL'); assert.equal(parsed[1].cnpj,'52998224725'); assert.equal(parsed[1].cnpjNormalizationStatus,'CPF_OR_AMBIGUOUS');
});