...
test('Posição 105 completa identifica colunas e totaliza produtos',()=>{
  ...
  for(let index=1;index<=50;index+=1)rows.push([index,`Produto ${index}`,10,12,2,3]);
  const products=parseStock105(rows,{byInternal:new Map(),factoryToInternal:new Map()});
  assert.equal(products.size,50); assert.equal(products.get('1')?.quantidade,10); assert.equal(products.get('1')?.vendaUnitario,3); assert.equal(products.get('1')?.unitsPerCase,12);
});

test('Posição 105 compacta é normalizada antes do parser e preserva Master/Un-CX',async()=>{
  const rows:any[][]=[];
  for(let index=1;index<=50;index+=1){const row=Array(16).fill('');row[0]=index;row[1]=`Produto ${index}`;row[4]=10;row[5]=12;row[6]=2;row[9]=3;rows.push(row)}
  ...
  assert.equal(products.size,50); assert.equal(products.get('50')?.custoUnitario,2); assert.equal(products.get('50')?.unitsPerCase,12);
});

test('Cadastro 286 completo reconhece código Winthor, fábrica, EAN válido e Master plausível',()=>{
...
test('Cadastro 286 não interpreta EAN/código longo como fator Un-CX',()=>{
  const ean=gtin13('789123456787');const row=Array(26).fill('');row[0]='11';row[1]='101';row[2]='Produto';row[20]=ean;row[23]='MATX';row[24]='7891234567895';
  const cadastro=parseCadastro286([row]);
  assert.equal(cadastro.byInternal.get('101')?.unitsPerCase,undefined);
});

test('Cadastro 286 compacto é expandido antes do parser e preserva Master plausível',async()=>{
