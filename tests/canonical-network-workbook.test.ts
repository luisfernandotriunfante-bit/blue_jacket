...
function state() {
  return {
    ...
    networks: [
      { key:'REDE A', name:'REDE A', networkTarget:1000, topTarget:600, invoiced:500, toInvoice:250, total:750, networkAttainment:.75, topAttainment:500/600, gapToNetworkTarget:250, gapToTopTarget:100, clients:2, stores:[{cnpj:'00123456000199',name:'LOJA A',fantasyName:'A',city:'CAMPO GRANDE',managerCnpj:'',groupingCode:'',tier:'OURO',storeType:'LOJA',routeNetwork:'ROTEIRO A',topTarget:600,invoiced:400,toInvoice:100,total:500}] },
...
test('workbook de Redes nasce apenas do estado canônico e exclui SEM REDE da planilha comercial',()=>{
  ...
  assert.equal(rows[0]['Realizado Tops'],500);
  assert.equal(rows[0]['% Meta Tops'],500/600);
});
...
test('workbook de Redes preserva loja Top e linhagem da fonte sem abas 319/12.326',()=>{
  ...
  assert.equal(stores[0]['Rede Roteiro'],'ROTEIRO A');
