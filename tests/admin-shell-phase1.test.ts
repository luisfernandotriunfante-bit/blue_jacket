...
test('sidebar principal tem exatamente sete módulos na ordem homologada', () => {
  assert.deepEqual(MAIN_SECTIONS.map(item => item.label), [
    'Sell Out',
    'PEX',
    'Estoque',
    'Atividades',
    'Clientes e Sortimento',
    'Documentos',
    'Administração',
  ]);
  assert.equal(MAIN_SECTIONS.length, 7);
  for (const removed of ['Metas', 'Listas Canônicas', 'Auditoria', 'Atualizar Bases']) {
    assert.equal(MAIN_SECTIONS.some(item => item.label === removed), false);
  }
});
...
test('Bases mantém as 19 fontes e não contém implementação de sync ou bundle', () => {
  ...
  assert.match(bases, /processSourceUpdates/);
  assert.match(bases, /REQUIRED_SOURCE_IDS\.map/);
...
test('Sincronização concentra pareamento, restore e Bundle sem tabela principal de fontes', () => {
  ...
  assert.match(sync, /connectDeviceSyncWorkspace/);
  assert.match(sync, /restoreCurrentDeviceSnapshot/);
  assert.match(sync, /recoverTechnicalBundle/);
  assert.match(sync, /Restaurar Bundle Canônico/);
...
test('Administração reutiliza os editores e visualizações oficiais sem duplicá-los', () => {
  ...
  assert.match(admin, /import \{ MetasPage \} from '\.\.\/MetasPage'/);
  assert.match(admin, /view === 'metas'\) return <MetasPage \/>/);
  assert.match(admin, /import \{ AuditoriaPage \} from '\.\.\/AuditoriaPage'/);
  assert.match(admin, /view === 'auditoria'\) return <AuditoriaPage \/>/);
  assert.match(admin, /import \{ ListasCanonicasPage \} from '\.\.\/ListasCanonicasPage'/);
  assert.match(admin, /view === 'canonical'\) return <ListasCanonicasPage \/>/);
...
test('deep-link mantém o mesmo hash sync e o shell não usa Configurações como destino', () => {
  const cloudSync = source('../src/canonical/cloudSync.ts');
  const main = source('../src/main.tsx');
  assert.match(cloudSync, /url\.hash = `sync=\$\{encodeURIComponent\(pairingCode\(identity\)\)\}`/);
  assert.match(cloudSync, /new URLSearchParams\(hash\)\.get\('sync'\)/);
  assert.match(main, /initialNavigationState\(window\.location\.hash\)/);
  assert.match(main, /activeTab === 'administracao'/);
